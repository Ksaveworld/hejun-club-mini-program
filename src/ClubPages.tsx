import { AmbassadorApplication } from './Ambassadors';
import { Campaign } from './Campaign';
export { ClubDirectory } from './Directory';
...
export function ClubHome() {
  return <div className="club-home"><Campaign auto/>
    <div className="club-logo"><img src="/images/cabc-official-logo.png" alt="中国—东盟经贸中心" width="316" height="86" /><a href="#/activities">活动与交流</a></div>
...
export function ClubMembers() {
  return <><Heading title="成为俱乐部会员" serif>三档会籍，按你的业务需求选择。</Heading><div className="club-plans">
    {plans.map(plan => <a href={`#/join/${plan.id}`} className={`club-plan club-plan-${plan.id}`} key={plan.id} aria-label={`${plan.name}，每年 ${money(plan.price)} 元，${plan.actionLabel}`}>
      ...
      <p>{plan.benefits.join(' · ')}</p><span className="club-plan-action">{plan.id === 'organization' ? '预报名，暂不收费' : '付款成功后开通'}<ClubIcon name="Right" variant="轮廓" size={16} /></span>
    </a>)}
...
export function ClubHelp() {
  const questions = [
    ['入会需要经过审核吗？', '基础会员和星级会员付款成功后开通，无需资质预审。机构及专业会员本期仅预报名，提交后待审核，暂不收费或开通。'],
