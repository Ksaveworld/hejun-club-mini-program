import { AmbassadorApplication, AdminAmbassadors } from './Ambassadors';
import { AdminSurveys } from './Surveys';
...
export function JoinMember({ plan, session, refresh }: { plan: Plan; session: Session; refresh: () => Promise<void> }) {
  ...
  return <><a className="back-link" href="#/members"><ArrowLeft size={17}/>返回会员权益</a>
    ...
    <form className="form-panel business-join" onSubmit={submit}>
      ...
      <div className="form-bottom"><span><strong>下一步</strong>{plan.id==='organization'?'等待资质审核':'查看待付款订单'}</span><button className="button primary" disabled={busy}>{busy?'保存中…':plan.id==='organization'?'提交预报名':'保存订单'}<ArrowRight size={17}/></button></div>
    </form></>;
}
...
export function OrderPage({ id, refresh }: { id: string; refresh: () => Promise<void> }) {
  ...
  return <div className="narrow-page"><a className="back-link" href="#/account?tab=orders"><ArrowLeft size={17}/>我的订单</a><div className="page-heading"><span className="eyebrow">会员订单</span><h1 id="page-title" tabIndex={-1}>{order?statusLabels[order.status]:'查看订单'}</h1></div>
    {error && <p className="field-error" role="alert">{error}</p>}
    {order && <section className="checkout-card"><div className="checkout-plan"><div><h2>{plans.find(p=>p.id===order.planId)?.name}</h2><p>{order.planId==='organization'?'预报名记录：本期暂不收费或开通会籍。':'付款成功后开通并起算有效期。'}</p></div><div className="price">{order.planId==='organization'?'暂不收费':<> <small>¥</small>{money(order.amountCents/100)}</>}</div></div>
      ...
      {order.status==='pending' && order.planId!=='organization' && <div className="payment-demo"><ShieldCheck size={22}/><div><strong>支付尚未开放</strong><p>微信支付账号尚未接通，订单已保存。当前不会收款或开通会员，接通并验收后才能付款。</p></div></div>}
...
export function AccountPage({session,refresh}:{session:Session;refresh:()=>Promise<void>}) {
  ...
  async function submit(event:FormEvent) {
    ...
    try{await api('/posts',{title,body},key.current);key.current=crypto.randomUUID();postFingerprint.current='';setTitle('');setBody('');setMessage('投稿已保存并发布，注册用户现在可以阅读。');await load();}catch(err){setError(errorText(err));}finally{setBusy(false);}
  }
...
export function AdminPage({session,refresh}:{session:Session;refresh:()=>Promise<void>}) {
  ...
  return <div className="admin-page"><a className="text-button" href="#/account"><ArrowLeft size={17}/>返回会员端</a><header className="admin-header"><div><p className="eyebrow">CLUB OPERATIONS</p><h1 id="page-title" tabIndex={-1}>运营工作台</h1><p className="admin-description">{isWebTrial?'服务器业务数据 · 微信支付尚未接通':'本机内测数据 · 微信支付尚未接通'}</p></div><button className="button secondary" onClick={()=>void load()}><RefreshCw size={16}/>刷新数据</button></header>
    <AdminSurveys/><AdminAmbassadors key={session.user.id}/><div className="business-stats">{[['注册账号',members.length],['待资质审核',orders.filter(o=>o.status==='review').length],['待付款',orders.filter(o=>o.status==='pending').length],['已付款订单',orders.filter(o=>o.status==='paid').length]].map(([label,value])=><section key={label}><span>{label}</span><strong>{value}</strong></section>)}</div>
